...
if (length(alpha_vars) > 0) {
  ...
  # 保存结果（英文版）
  write.csv(alpha_by_cluster, file.path(RESULTS_DIR, "table_S2_alpha_by_cluster_P.csv"), row.names = FALSE)
  cat(" ✅ Table S2 saved: table_S2_alpha_by_cluster_P.csv\n")
  # 统计检验
...
cat("\n")
if (file.exists(file.path(RESULTS_DIR, "table_S2_alpha_by_cluster_P.csv"))) {
  alpha_tab <- read.csv(file.path(RESULTS_DIR, "table_S2_alpha_by_cluster_P.csv"))
